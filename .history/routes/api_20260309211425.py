...
# ==========================================
IPS_CONFIAVEIS = os.getenv('IPS_PROXY_CONFIAVEIS', '').split(',')

def obter_ip_real(request: Request) -> str:
    """Extrai o IP real do cliente de forma segura.
    Só confia no X-Forwarded-For se a conexão vier de um proxy conhecido.
    Caso contrário, usa o IP da conexão TCP (impossível de spoofar)."""
    ip_conexao = request.client.host if request.client else '0.0.0.0'
    
    xff = request.headers.get('X-Forwarded-For')
    if xff and ip_conexao in IPS_CONFIAVEIS:
        # Proxy confiável: pega o PRIMEIRO IP da cadeia (o cliente original)
        return xff.split(',')[0].strip()
    
    # Sem proxy confiável ou sem header: IP real da conexão TCP
    return ip_conexao
